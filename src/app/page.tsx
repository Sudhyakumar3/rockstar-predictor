    ...
    <div className="relative flex flex-col items-center min-h-screen bg-gray-900 text-white px-4">
      ...
      <AnimatePresence>
        {showResults && (
          ...
          >
            <motion.div
              className="bg-gray-900 bg-opacity-80 text-white p-8 w-full max-w-5xl max-h-[80vh] shadow-lg overflow-y-auto relative results-popup border border-gray-600"
              variants={popupVariants} // Apply the same variants to the content container
            >

              {/* Close Button */}
              <button
                onClick={() => setShowResults(false)} // Directly set setShowResults to false
